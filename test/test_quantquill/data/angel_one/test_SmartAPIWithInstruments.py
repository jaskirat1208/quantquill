...
from quantquill.data.angel_one.utils.constants import INSTRUMENTS_CACHE_PATH, INSTRUMENTS_URL
...
class TestSmartConnect(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch('quantquill.data.angel_one.utils.SmartAPIWithInstruments.api.SmartConnect.__init__'), \
    ...
    @patch('quantquill.data.angel_one.utils.SmartAPIWithInstruments.os.path.exists')
    @patch('quantquill.data.angel_one.utils.SmartAPIWithInstruments.datetime')
    ...
    @patch('quantquill.data.angel_one.utils.SmartAPIWithInstruments.os.makedirs')
    @patch('quantquill.data.angel_one.utils.SmartAPIWithInstruments.os.path.exists')
    @patch('quantquill.data.angel_one.utils.SmartAPIWithInstruments.datetime')
    ...
    @patch('quantquill.data.angel_one.utils.SmartAPIWithInstruments.requests.get')
...
class TestSmartConnectIntegration(unittest.TestCase):
    ...
    @patch('quantquill.data.angel_one.utils.SmartAPIWithInstruments.api.SmartConnect.__init__')
    def test_full_loadInstruments_workflow(self, mock_parent_init):
        ...
        with patch('quantquill.data.angel_one.utils.SmartAPIWithInstruments.INSTRUMENTS_CACHE_PATH', self.temp_dir + '/'):
            with patch.object(SmartConnect, '_fetchInstrumentsFromAPI') as mock_fetch:
                mock_fetch.return_value = [
                    {"token": "99926000", "symbol": "NIFTY 50", "name": "NIFTY"}
                ]
                
                with patch('quantquill.data.angel_one.utils.SmartAPIWithInstruments.datetime') as mock_datetime:
                    mock_datetime.now.return_value.strftime.return_value = '20231201'
